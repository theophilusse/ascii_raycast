...
void	renderSpriteProjection(char *colorBuffer, upng_t **textures)
{
	...
	for (int i = 0; i < numVisibleSprites; i++)
	{
		...
		// Sprite top Y
		spriteTopY = (LINES / 2) - (spriteHeight / 2);
		spriteTopY = (spriteTopY < 0) ? 0 : spriteTopY;

		// Sprite bottom Y
		spriteBottomY = (LINES / 2) + (spriteHeight / 2);
		spriteBottomY = (spriteBottomY > LINES) ? LINES : spriteBottomY;
		...
		// SpriteLeftX
		spriteLeftX = (COLS / 2) + spriteScreenPoX - (spriteWidth / 2);
		...
		for (int x = spriteLeftX; x < spriteRightX; x++)
		{
			...
			for (int y = spriteTopY; y < spriteBottomY; y++)
			{
				if (x > 0 && x < COLS && y > 0 && y < LINES)
				{
					...
					distanceFromTop = y + (spriteHeight / 2) - (LINES / 2);
